...
export class WeChatAdapter implements IPlatformAdapter {
    ...
    async showRewardedVideoAd(adUnitId: string): Promise<{ success: boolean; rewarded: boolean }> {
        return new Promise((resolve) => {
            if (!this.isWeChatAvailable()) { resolve({ success: false, rewarded: false }); return; }
            try {
                const videoAd = wx.createRewardedVideoAd({ adUnitId });
                videoAd.onLoad(() => console.log('激励视频广告加载成功'));
                videoAd.onError((err: any) => resolve({ success: false, rewarded: false }));
                videoAd.onClose((res: any) => resolve({ success: true, rewarded: res.isEnded || false }));
                videoAd.show().catch((err: any) => resolve({ success: false, rewarded: false }));
            } catch (error) {
    ...
    async showInterstitialAd(adUnitId: string): Promise<boolean> {
        return new Promise((resolve) => {
            if (!this.isWeChatAvailable()) { resolve(false); return; }
            try {
    ...
    async login(): Promise<{ success: boolean; code?: string; userInfo?: any }> {
        return new Promise((resolve) => {
            if (!this.isWeChatAvailable()) { resolve({ success: false }); return; }
            wx.login({
    ...
    async createOrder(options: { productId: string; productName: string; price: number }): Promise<{ success: boolean; orderId?: string }> {
        return Promise.resolve({ success: true, orderId: `wx_order_${Date.now()}` });
    }
    
    async trackEvent(eventName: string, data?: any): Promise<void> {
        if (!this.isWeChatAvailable()) return;
        try { wx.reportAnalytics(eventName, data || {}); } catch (error) { /* ignore */ }
        return Promise.resolve();
    }
    ...
    async getSystemInfo(): Promise<{
        ...
        return new Promise((resolve) => {
            if (!this.isWeChatAvailable()) { resolve(this.getDefaultSystemInfo()); return; }
            wx.getSystemInfo({
                success: (res: any) => resolve({
                    platform: 'wechat', version: res.version || '1.0.0', SDKVersion: res.SDKVersion || '1.0.0',
    ...
    private getDefaultSystemInfo() {
        return {
            platform: 'wechat', version: '1.0.0', SDKVersion: '1.0.0',
    ...
    async vibrateShort(): Promise<void> {
        if (this.isWeChatAvailable()) try { wx.vibrateShort({}); } catch (e) { /* ignore */ }
        return Promise.resolve();
    }
    
    async vibrateLong(): Promise<void> {
        if (this.isWeChatAvailable()) try { wx.vibrateLong({}); } catch (e) { /* ignore */ }
        return Promise.resolve();
    }
    
    async setStorage(key: string, data: any): Promise<void> {
        if (!this.isWeChatAvailable()) return;
        try { wx.setStorageSync(key, data); } catch (error) { console.error(`WeChatAdapter: 存储失败 (key=${key}):`, error); }
    }
